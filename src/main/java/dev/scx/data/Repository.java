...
public interface Repository<Entity, ID> {
    ...
    /// 添加数据
    ///
    /// 若 entity 为 null, 则相当于仅使用 fieldPolicy 进行纯表达式添加
    ///
    ...
    ///
    /// @param entityList  待添加的数据列表 (成员可为 null)
    /// @param fieldPolicy 字段策略
    /// @return 主键 ID 列表 (若数据没有主键, 则列表元素为 null)
    ...
    /// 更新数据
    ///
    /// 若 entity 为 null, 则相当于仅使用 fieldPolicy 进行纯表达式更新, 此时要求 fieldPolicy 至少包含一个字段表达式
    ///
